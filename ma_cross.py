...
class MarketOnClosePortfolio(Portfolio):
    ...
    def backtest_portfolio(self):
        spos = self.positions[self.symbol]
        pfor = spos * self.bars['Close']

        #portfolio = self.positions*self.bars['Close']
        portfolio = pd.DataFrame(data=pfor, columns=['marketprice'])
        portfolio.to_csv('portfolio')
        
        pos_diff = self.positions.diff()
        pos_diff.to_csv('pos_diff')
        
        # portfolio['holdings'] = (self.positions*self.bars['Close']).sum(axis=1)
        portfolio['holdings'] = pd.DataFrame(pfor).sum(axis=1,level=0)

        #portfolio['cash'] = self.initial_capital - (pos_diff*self.bars['Close']).sum(axis=1).cumsum()
        jgz = pd.DataFrame(pos_diff[self.symbol]*self.bars['Close'])
        
        portfolio['cash'] = self.initial_capital - (jgz).sum(axis=1,level=0).cumsum()
...
if __name__ == "__main__":
    ...
    mac = MovingAverageCrossStrategy(symbol, bars, short_window=10, long_window=30)
